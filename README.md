...
|------|---------|
| 密码强度检测 | 分析密码强度与安全建议 |
| 哈希值计算 | SHA-1/256/384/512 哈希计算 |
| 凯撒密码 | 字母位移加密解密工具 |
| ROT13 编解码 | ROT13 文本双向转换 |
| JWT 令牌解析 | 解析 JWT 头部与载荷内容 |
| UUID 生成器 | 批量生成随机 UUID v4 |
| 摩尔斯电码 | 文本与摩尔斯电码互转 |
| 文本转二进制 | 文本与 8 位二进制编码互转 |
| 随机令牌生成 | 生成随机 API 密钥令牌 |
| XOR 异或加密 | 密钥 XOR 位运算加密演示 |

### 📡 物联网工具类 (8)

| 工具 | 功能描述 |
|------|---------|
| MQTT 主题验证器 | MQTT 主题格式验证与层级解析 |
| CRC 校验计算器 | CRC-8/16/CCITT/32 校验值计算 |
| 字节/进制转换器 | 十六进制、十进制、二进制、ASCII 互转 |
| IP 子网计算器 | CIDR 子网掩码、主机数量计算 |
| Unix 时间戳转换 | 时间戳与日期时间双向转换 |
| 位运算计算器 | AND/OR/XOR/NOT/移位运算 |
| 设备功耗计算器 | 估算 IoT 设备电池续航时间 |
| 波特率计算器 | UART/RS-485 串口数据传输计算 |
...
| 工具 | 功能描述 |
|------|---------|
| 视觉记忆测试 | 记住高亮格子的位置，逐级增加难度 |
| 数字记忆测试 | 记忆越来越长的随机数字序列 |
| 鼠标精确点击 | 点击越来越小的圆形目标，测试鼠标精准度 |
| 色盲检测 | 辨别有色点图中隐藏的数字，评估色觉状态 |
| 瞄准训练 | 30 秒内快速点击随机目标，统计得分与反应时间 |
